...
SleepTracker::SleepTracker()
    : _state(STATE_IDLE), _sleepStartMs(0), _confirmStartMs(0)
{
...
void SleepTracker::stopSleep(float avgTemp, float avgHumidity) {
    if (_state != STATE_TRACKING && _state != STATE_CONFIRM_STOP) return;
...
unsigned long SleepTracker::getElapsedMs() const {
    if (_state == STATE_IDLE || _state == STATE_STOPPED) return 0;
...
SleepPhase SleepTracker::getCurrentPhase() const {
    ...
    if (mins < 5)   return PHASE_AWAKE;
    if (cycle < 30) return PHASE_LIGHT;
    if (cycle < 60) return PHASE_DEEP;
    return PHASE_REM;
}
...
const char* SleepTracker::getPhaseName() const {
    switch (getCurrentPhase()) {
        ...
        case PHASE_LIGHT: return "Light";
        case PHASE_DEEP:  return "Deep";
        case PHASE_REM:   return "REM";
        default:          return "---";
    }
}
...
SleepSession SleepTracker::getHistory(int index) const {
    if (index < 0 || index >= HISTORY_COUNT) {
        SleepSession empty; empty.valid = false; return empty;
    }
